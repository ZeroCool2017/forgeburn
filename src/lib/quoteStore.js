...
export const QUOTE_GROUPS = [
  ...
  { id: 'big_sean', label: 'Big Sean / It\'s So Cold in the D', filter: q => q.author?.includes('Big Sean') || q.author?.includes('It\'s So Cold') },
  { id: 'deee_lite', label: 'Deee-Lite', filter: q => q.author?.includes('Deee-Lite') },
  { id: 'weezer_blue', label: 'Weezer / Blue Album', filter: q => q.author?.includes('Weezer') || q.author?.includes('Blue Album') },
  { id: 'cibo_matto', label: 'Cibo Matto', filter: q => q.author?.includes('Cibo Matto') },
  { id: 'cranberries', label: 'The Cranberries / Dolores O\'Riordan', filter: q => q.author?.includes('Cranberries') || q.author?.includes('Dolores O\'Riordan') },
  { id: 'sgi_more', label: 'SGI / Daisaku Ikeda (extended)', filter: q => q.author?.includes('Daisaku Ikeda') || q.author?.includes('SGI') },
  { id: 'atlanta_business', label: 'Atlanta Business Owners / Andre 3000', filter: q => q.author?.includes('Magic City') || q.author?.includes('Andre 3000') || q.author?.includes('Atlanta') },
  { id: 'mia', label: 'MIA', filter: q => q.author?.includes('MIA') },
  { id: 'gemma_chan', label: 'Gemma Chan', filter: q => q.author?.includes('Gemma Chan') },
  { id: 'erykah_badu', label: 'Erykah Badu', filter: q => q.author?.includes('Erykah Badu') },
  { id: 'ds_games', label: 'DS Games (Brain Age, Tetris)', filter: q => q.author?.includes('Brain Age') || q.author?.includes('Tetris') || q.author?.includes('Nintendo DS') },
  { id: 'chess', label: 'Chess Prodigies & Masters', filter: q => q.author?.includes('Fischer') || q.author?.includes('Carlsen') || q.author?.includes('Capablanca') || q.author?.includes('Nimzowitsch') || q.author?.includes('Chess') },
